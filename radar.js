...
d[1].reverse()
	...
	d.forEach(function(y, x){
      ...
      var colors = ['#cdf4c1','#ffdf8f','#ffa1a1','#b3e5fc','#b3e5fc', '#b3e5fc', '#b3e5fc']
      var idColor = Math.round(Math.max(j.value, 0)*10) - 1;
      idColor = (idColor<0? 0 : idColor)
      if (series <= 0 ){
        var nwColor = colorWar
        nwColor.reverse()
        // console.log(nwColor[i], i)
        // debugger
        return nwColor[i]
      }else{
        return colors[idColor]
      }
      // console.log(Math.round(Math.max(j.value, 0)*10) - 1 <0 : 0 ? Math.round(Math.max(j.value, 0)*10) - 1)
